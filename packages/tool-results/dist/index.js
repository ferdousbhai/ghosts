...
export function sliceTextPage(value, contentStart, maxCharacters) {
    let start = Math.min(Math.max(0, Number.isSafeInteger(contentStart) ? contentStart : 0), value.length);
    if (start > 0 &&
        start < value.length &&
...
export function formatToolResultPage(snapshot, contentStart = 0, maxCharacters = TOOL_RESULT_PAGE_DEFAULT_CHARACTERS) {
    return formatStoredToolResultPage({
        ...snapshot,
        ...toolResultPage(snapshot, contentStart, maxCharacters),
    });
}
export function formatStoredToolResultPage(page) {
 ...
 */
export function createToolResultBoundary(store, maximumTurnCharacters = TOOL_RESULT_TURN_MAX_CHARACTERS, scope = `tool-result-scope-${crypto.randomUUID()}`, maximumRetainedCharacters = TOOL_RESULT_BOUNDARY_RETAINED_MAX_CHARACTERS) {
    const turnLimit = boundedInteger(maximumTurnCharacters, TOOL_RESULT_TURN_MAX_CHARACTERS, TOOL_RESULT_TURN_MAX_CHARACTERS);
    ...
    return {
        ...
        deliver({ content, toolCallId, toolName }) {
            ...
            const consumed = consume(fitStoredToolResultPage(firstPage, turnLimit - deliveredCharacters));
...
export function boundedToolErrorMessage(error) {
    let message;
    try {
    ...
    if (message.length <= TOOL_ERROR_MAX_CHARACTERS)
        return message;
...
export function createInMemoryToolResultStore(maximumCharacters = TOOL_RESULT_SNAPSHOT_MAX_CHARACTERS, maximumTotalCharacters = TOOL_RESULT_STORE_MAX_CHARACTERS) {
    ...
    const get = (handle) => {
        ...
        if (!snapshot)
            return null;
        if (snapshot.createdAt < Date.now() - TOOL_RESULT_SNAPSHOT_TTL_MS &&
    ...
    return {
        ...
        set(input) {
            ...
            if (projectedCharacters > maximumTotalCharacters)
                return null;
            if (entries.size >= TOOL_RESULT_SNAPSHOT_MAX_ENTRIES) {
                const evicted = [...entries].find(([, snapshot]) => !pinnedScopes.has(snapshot.scope));
                if (!evicted)
                    return null;
        ...
        pinScope(scope) {
            ...
            return () => {
                ...
                if (remaining > 0)
                    pinnedScopes.set(scope, remaining);
                else
                    pinnedScopes.delete(scope);
            };
        },
        clear() {
            entries.clear();
...
function previousPageStart(value, contentStart, maxCharacters) {
...
function boundedMetadata(value, maxCharacters = 200) {
    return sliceTextPage(toWellFormedText(value), 0, maxCharacters).content;
}
function xmlAttribute(value, maxCharacters = 200) {
...
function fitStoredToolResultPage(page, maximumOutputCharacters) {
    let low = 0;
    ...
    return renderStoredToolResultPage(pageWithContent(page, localPage.content, localPage.contentEnd));
}
function pageWithContent(page, content, localContentEnd) {
...
function compactToolDetails(input) {
    return Object.freeze({
        paginated: true,
        handle: input.handle,
...
function outputBudgetMessage(limit) {
    return [
...
function unavailableResultMessage(toolName, totalCharacters) {
    return [
