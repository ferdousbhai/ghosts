...
- a bounded, expiring in-memory store for process-local use.

It does not include agents, Durable Objects, authentication, authorization, billing, or product policy. A host that needs durable persistence implements `ToolResultStore` and injects it into `createToolResultBoundary`.
...
Small results are returned directly. Oversized results are normalized to well-formed text, retained through the injected store, and represented by a bounded first page. If persistence fails, delivery fails closed with a terminal unavailable result rather than encouraging a retry of a possibly side-effecting tool.

## Storage contract and security

`ToolResultStore.set` stores the exact normalized snapshot and returns `null` when it cannot safely retain it. A returned handle must remain bound to that exact content, metadata, and scope for its lifetime; retries create a new handle rather than changing an exposed snapshot. `getPage` is the redemption boundary: it must return `null` unless both the opaque handle and private scope match. `pinScope`, when implemented, prevents active-run snapshots from expiring.

The package's in-memory adapter enforces per-entry, aggregate, count, and TTL limits. Durable adapters must enforce their own equivalent retention limits and atomicity. Never expose scopes to the model or treat a handle alone as authorization. Host applications remain responsible for caller authorization and tenant isolation.
