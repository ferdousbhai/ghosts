import { z } from "zod";
export const WEB_SEARCH_MODEL_MAX_RESULTS = 6;
export const WEB_SEARCH_MODEL_EXCERPT_MAX_CHARACTERS = 400;
export const WEB_SEARCH_MODEL_MAX_CHARACTERS = 6_000;
export const READ_URL_MODEL_MAX_RESULTS = 5;
export const READ_URL_MODEL_CONTENT_MAX_CHARACTERS = 12_000;
export const READ_URL_MODEL_MAX_CHARACTERS = 60_000;
...
export function formatWebSearchResults(results) {
    ...
    const entries = results
        ...
        const excerpt = truncateText(compactHighlights(result.highlights) ||
            compactBoundedText(result.summary, WEB_SEARCH_MODEL_EXCERPT_MAX_CHARACTERS) ||
            compactBoundedText(result.text, WEB_SEARCH_MODEL_EXCERPT_MAX_CHARACTERS), WEB_SEARCH_MODEL_EXCERPT_MAX_CHARACTERS);
        return excerpt ? `${heading}\n> ${excerpt}` : heading;
    });
    return joinBoundedResults(entries, WEB_SEARCH_MODEL_MAX_CHARACTERS);
}
export function formatReadUrlResults(results) {
    if (results.length === 0)
    ...
    const entries = results
        ...
        .map((result, index) => {
        const heading = formatMarkdownResultHeading(result, index);
        const content = compactBoundedText(result.text ?? result.summary, READ_URL_MODEL_CONTENT_MAX_CHARACTERS) ||
            "No content extracted.";
        return `${heading}\n${content}`;
    });
    return joinBoundedResults(entries, READ_URL_MODEL_MAX_CHARACTERS);
}
function formatMarkdownResultHeading(result, index) {
...
function compactHighlights(values) {
    if (!values)
        return "";
    const highlights = [];
    let length = 0;
...
function compactBoundedText(value, maxCharacters) {
    return compactBoundedTextResult(value, maxCharacters).text;
}
function compactBoundedTextResult(value, maxCharacters) {
...
function indicateTruncation(value, maxCharacters) {
    if (maxCharacters <= 0)
        return "";
    return `${sliceText(value, maxCharacters - 1).trimEnd()}…`;
}
function joinBoundedResults(entries, maxCharacters) {
    let output = "";
    for (const entry of entries) {
        const separator = output ? "\n\n" : "";
        if (output.length + separator.length + entry.length > maxCharacters)
            break;
        output += separator + entry;
    }
    return output;
}
function sliceText(value, maxCharacters) {
...
function escapeMarkdownLabel(value, maxCharacters) {
    let escaped = "";
...
function escapeMarkdownUrl(value, maxCharacters) {
    let escaped = "";
    for (const character of value) {
        const next = character === "(" ? "%28" : character === ")" ? "%29" : character;
        if (escaped.length + next.length > maxCharacters)
            break;
        escaped += next;
    }
    return escaped;
