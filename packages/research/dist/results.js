...
export function formatWebSearchResults(results) {
    ...
    return results
        ...
        const excerpt = truncateText(result.highlights?.map(compactText).filter(Boolean).join(" ") ||
            compactText(result.summary) ||
            compactText(result.text), WEB_SEARCH_MODEL_EXCERPT_MAX_CHARACTERS);
...
export function formatReadUrlResults(results) {
    ...
    return results
        ...
        .map((result, index) => {
        const heading = formatMarkdownResultHeading(result, index);
        const content = truncateText(compactText(result.text ?? result.summary) || "No content extracted.", READ_URL_MODEL_CONTENT_MAX_CHARACTERS);
        return `${heading}\n${content}`;
    })
        .join("\n\n");
}
