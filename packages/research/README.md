...
Portable research primitives shared by `summon-ghost` and `ghost-build`:
...
- The shared `web_search` contract accepts multiple queries. Consumers using `summon-ghost`'s former `{ query }`-only schema may continue passing just `query`, but parsed defaults are now present.
...
- `formatWebSearchResults` uses the `summon-ghost` bounded Markdown contract and accepts only `results`. Consumers migrating from a leading `query` argument must remove it and should not expect XML. Both Markdown formatters bound each rendered field and the aggregate output; exported `*_MAX_CHARACTERS` constants document those limits. Invalid calendar dates are omitted from formatted metadata.
