...
export const X_SEARCH_TOOL_NAME = "x_search" as const;
...
export const readUrlInputSchema = z.strictObject({
  urls: z
    .array(
      z.url().max(4_096).superRefine((value, context) => {
        try {
...
export function normalizeSearchQueries(input: Pick<WebSearchInput, "query" | "queries">): string[] {
