...
export type ResearchBoundaryErrorCode =
  | "InvalidPublicUrl"
  | "PublicUrlProtocolRejected"
  | "PublicUrlCredentialsRejected"
  | "PublicUrlDestinationRejected"
  | "InvalidPublicUrlResponseLimit"
  | "PublicUrlResponseTooLarge";
...
export function assertPublicHttpsUrl(value: string | URL): URL {
  ...
  const hostname = url.hostname
    ...
    .replace(/\.$/, "");
  if (
    FORBIDDEN_PUBLIC_HOSTS.has(hostname) ||
