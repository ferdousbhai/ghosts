...
export type InferToolInput<TSchema> =
  TSchema extends StandardSchemaV1<infer TInput>
    ? TInput
    : TSchema extends ZodSchemaLike<infer TInput>
      ? TInput
      : unknown;

export type ToolDefinitionForSchema<
  TSchema extends ToolInputSchema,
  TOutput = unknown,
> = Omit<ToolDefinition<InferToolInput<TSchema>, TOutput>, "inputSchema"> &
...
export function defineTool<
  ...
  TOutput = unknown,
>(
  definition: ToolDefinitionForSchema<TSchema, TOutput>,
): ToolDefinitionForSchema<TSchema, TOutput> {
  return definition;
}
 ...
 */
export function withModelJsonSchema<TInput = unknown>(
  validationSchema: ToolInputSchema,
  modelSchema: Draft07JsonSchema,
  ...
  return Object.freeze({
    "~standard": Object.freeze({
      ...
      validate: async (value: unknown): Promise<StandardSchemaResult<TInput>> => {
        try {
          return {
            value: await validateToolArguments<TInput>(validationSchema, value),
          };
        } catch (error) {
      ...
      jsonSchema: Object.freeze({
        input: ({ target }: Readonly<{ target: "draft-07" }>) => {
          if (target !== "draft-07") {
